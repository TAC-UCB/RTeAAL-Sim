...
object Emitter {
  ...
  def emitExpr(e: Expression)(implicit rn: Renamer = null): String = e match {
    ...
    case p: DoPrim => p.op match {
      ...
      case Rem =>
        p.args.zipWithIndex.foreach { case (a, i) =>
          println(s"[DEBUG] Rem arg $i: $a")
        }
  ...
  def emitStmt(s: Statement)(implicit rn: Renamer): Seq[String] = s match {
    ...
    case d: DefNode => {
      ...
      if (rn.decLocal(lhs_orig)) {
        val cppType = genCppType(d.value.tpe)
        if (rem_flag) {
          Seq(s"$cppType $lhs = (${rhs}).bits<${bitWidth(d.value.tpe)-1}, 0>();")
        }
        else Seq(s"$cppType $lhs = $rhs;")
      }
      else Seq(s"$lhs = $rhs;")
    }
    case c: Connect => {

...
ALWAYS_INLINE uint16_t op_ss_for_loop(
    const uint32_t* __restrict& rptr,
    const uint16_t* __restrict& fptr,
    ...
    // 5: ltS
    {
        ...
        for (uint32_t i = start; i < end; ++i) {
            LR(l, r);
            int64_t lhs = static_cast<int64_t>(databuf[l]);
            int64_t rhs = static_cast<int64_t>(databuf[r]);
            output_buffer[i] = lhs < rhs;
        }
    }
    ...
    // 10: shl
    {
        ...
        for (uint32_t i = start; i < end; ++i) {
            LR(l, r);
            uint64_t val = databuf[l];
            uint64_t shift = databuf[r];
    ...
    // 12: shrS
    {
        ...
        for (uint32_t i = start; i < end; ++i) {
            LR(l, r);
            int64_t val = static_cast<int64_t>(databuf[l]);
            uint64_t shift = databuf[r];
    ...
    // 14: or
    {
        ...
        for (; i < limit; i += 8) {
            ...
            LR(l, r);
            uint64_t lhs0 = databuf[l];
            uint64_t rhs0 = databuf[r];
            output_buffer[i] = lhs0 | rhs0;
    ...
    // 16: parity (popcount modulo 2) xorr
    {
        READ_BOUNDS(start, end);
        for (uint32_t i = start; i < end; ++i) {
    ...
    // 18: bits
    {
        ...
        //     uint64_t y = *rptr++;
        //     uint64_t z = *rptr++;
        //     output_buffer[i] = (databuf[x] >> y) & databuf[z];
        // }
        uint32_t i = start;
        uint32_t limit = end - ((end - start) & 7);
        for (; i < limit; i += 8) {
            ...
            }
            {
                ...
                LR(x, y);
                uint64_t z = *rptr++;
                output_buffer[i + 1] = (databuf[x] >> y) & databuf[z];
            }
            {
                // size_t x = *rptr++;
                // uint32_t y = *rptr++;
                LR(x, y);
                uint64_t z = *rptr++;
                output_buffer[i + 2] = (databuf[x] >> y) & databuf[z];
            }
            {
                // size_t x = *rptr++;
                // uint32_t y = *rptr++;
                LR(x, y);
                uint64_t z = *rptr++;
                output_buffer[i + 3] = (databuf[x] >> y) & databuf[z];
            }
            {
                // size_t x = *rptr++;
                // uint32_t y = *rptr++;
                LR(x, y);
                uint64_t z = *rptr++;
                output_buffer[i + 4] = (databuf[x] >> y) & databuf[z];
            }
            {
                // size_t x = *rptr++;
                // uint32_t y = *rptr++;
                LR(x, y);
                uint64_t z = *rptr++;
                output_buffer[i + 5] = (databuf[x] >> y) & databuf[z];
            }
            {
                // size_t x = *rptr++;
                // uint32_t y = *rptr++;
                LR(x, y);
                uint64_t z = *rptr++;
                output_buffer[i + 6] = (databuf[x] >> y) & databuf[z];
            }
            {
                // size_t x = *rptr++;
                // uint32_t y = *rptr++;
                LR(x, y);
                uint64_t z = *rptr++;
                output_buffer[i + 7] = (databuf[x] >> y) & databuf[z];
            }
        }
    ...
    // 19: mux
    {
        ...
        //     output_buffer[i] = databuf[*x] > 0 ? databuf[*y] : databuf[*z];
        // }
        uint32_t i = start;
    ...
    // 24: mux chain
    {
        ...
        for (uint32_t i = start; i < end; ++i) {
            ...
            // std::cout << "databuf[y]: " << databuf[y] << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    std::vector<uint16_t> all_len = loadVectorFromJson<uint16_t>(JSONPATH + prefix+"_fptr.json");
    ...
    while (cycle < sim_cycles) {
        ...
        const uint16_t* __restrict fptr = all_len.data();
        const uint32_t* __restrict s_ptr = new_s.data();

        rptr = (const uint32_t*)__builtin_assume_aligned(rptr, 64);
        fptr = (const uint16_t*)__builtin_assume_aligned(fptr, 64);
        ...
        for (size_t i = 0; i < layer_num; ++i) {
            // printf("Layer: %zu\n", i);
            layer_len = op_ss_for_loop( rptr, fptr, databuf_ptr, outbuf_ptr, memory_64_ptr, memory_8_ptr, muxJT_ptr, scratch1, scratch2);
            size_t j = 0;
            for (; j + 23 < layer_len; j += 24) {
                COPY24();
            }

...
public:
  ...
  void rteaal_rand_init() {
    rteaal_core_rand_init();
    mask_top_unused();
  }
  ...
  uint64_t fetch_word_index(uint32_t index) const {
    if (index >= n_) {
      std::cerr << "ERROR: fetch_word_index out of bounds: " << index << " >= " << n_ << std::endl;
      std::exit(-1);
    }
    return words_[index];
  }
...
private:
  ...
  void core_rand_init() {
    ...
    if (w_ < 64) {
      ...
      for (int word=0; word < n_; word++) {
        words_[word] = rng64();  // rng64()
      }
    }
  }

  __attribute__((noinline))
  void rteaal_core_rand_init() {
    // trusting mask_top_unused() will be called afterwards
    if (w_ < 64) {
      if (w_ > rng_bits_left) {
        rng_leftover = rng64();  // rng64()

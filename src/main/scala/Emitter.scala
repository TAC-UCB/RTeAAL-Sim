...
object Emitter {
  ...
  def bitWidthFromCppType(cppType: String): Int = {
    val Pattern = raw"(U|S)Int<(\d+)>".r
    cppType match {
      case Pattern(_, width) => width.toInt
      case _ => throw new Exception(s"Unknown type format: $cppType")
    }
  }
  ...
  def splatLargeLiteralIntoRawArray(value: BigInt, width: BigInt): String = {
    ...
    val chunks = chunkLitString(asHexStr)
    val arrStr = chunks.map("0x" + _).mkString(",")
    val leadingNegStr = if (isNeg) "(uint64_t) -" else ""

    val rawWordCount = chunks.length
    val idealWordCount = ((width + 63) / 64).toInt

    val needsPadding = rawWordCount < idealWordCount
    val numWords = if (needsPadding) rawWordCount else idealWordCount

    val padStr = if (needsPadding) s".pad<$width>()" else ""
    val numbits = if (needsPadding) rawWordCount * 64 else width

    s"<$numbits>(std::array<uint64_t,$numWords>({$leadingNegStr$arrStr}))$padStr"
  }
  ...
  def emitExpr(e: Expression)(implicit rn: Renamer = null): String = e match {
    ...
    case u: UIntLiteral => {
      ...
      else s"UInt${splatLargeLiteralIntoRawArray(u.value, width)}"
    }
    case u: SIntLiteral => {
      val width = bitWidth(u.tpe)
      if (width <= 64) s"SInt<$width>(${u.value.toString(10)})"
      else s"SInt${splatLargeLiteralIntoRawArray(u.value, width)}"
    }
    case m: Mux => {
